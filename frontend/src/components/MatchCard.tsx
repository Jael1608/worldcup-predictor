...
const penaltyPredictionStages = new Set(["ROUND_OF_16", "QUARTER_FINAL", "SEMI_FINAL", "THIRD_PLACE", "FINAL"]);
export const MatchCard = ({ match, onSaved }: { match: Match; onSaved: () => void }) => {
  const { formatDateTime } = useUserDateTime();
  const [home, setHome] = useState(""); const [away, setAway] = useState(""); const [penaltyWinner, setPenaltyWinner] = useState(""); const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  const requiresPenaltyWinner = penaltyPredictionStages.has(match.stage);
  const save = async (event: FormEvent) => {
    event.preventDefault(); setSaving(true); setError("");
    try { await api.post("/predictions", { matchId: match.id, predictedHome: Number(home), predictedAway: Number(away), predictedPenaltyWinner: requiresPenaltyWinner ? penaltyWinner : undefined }); onSaved(); }
  ...
  return <article className="panel">
    ...
    {match.myPrediction ? <div className="mt-4 rounded-xl border border-[#30415d] bg-[#16243a] p-3 text-center"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Predicción guardada · bloqueada</p><p className="mt-1 text-xl font-black">{match.myPrediction.predictedHome} - {match.myPrediction.predictedAway}</p>{match.myPrediction.predictedPenaltyWinner && <p className="mt-1 text-xs font-bold text-yellow-200">En caso de penales: {match.myPrediction.predictedPenaltyWinner}</p>}{match.status === "FINISHED" && <p className="text-sm text-green-300">{match.myPrediction.points} puntos</p>}</div>
    : match.canPredict ? <form className="mt-4" onSubmit={save}><div className="flex items-center justify-center gap-3"><input required min="0" className="w-16 text-center" type="number" value={home} onChange={(e) => setHome(e.target.value)} /><span>-</span><input required min="0" className="w-16 text-center" type="number" value={away} onChange={(e) => setAway(e.target.value)} /></div>{requiresPenaltyWinner && <div className="mt-3"><label className="mb-1 block text-xs font-black uppercase tracking-wider text-yellow-200">En caso de penales</label><select required className="w-full" value={penaltyWinner} onChange={(e) => setPenaltyWinner(e.target.value)}><option value="">Seleccionar quién pasa</option><option value={match.homeTeam}>{match.homeTeam}</option><option value={match.awayTeam}>{match.awayTeam}</option></select></div>}<button className="button-primary mt-3 w-full" disabled={saving || home === "" || away === "" || (requiresPenaltyWinner && !penaltyWinner)}>{saving ? "Guardando..." : "Guardar predicción"}</button>{error && <p className="mt-2 text-sm text-red-300">{error}</p>}</form>
